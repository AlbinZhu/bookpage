...
class PolyGonHeatmapGenerator(BaseHeatmapGenerator):
    ...
    def generate_heatmap_anno(self, annos):
        batches = len(annos)
        target_batch_annos = np.zeros(
            (self.heat_h, self.heat_w, self.num_joints), dtype=np.float32)
        for _, (points,
                label) in enumerate(zip(annos['points'], annos['labels'])):
            if label == 0:
                start_index = 0
                end_index = 4
            elif label == 1:
            ...
            heat_map = self.get_heatmap(polygon_points, self.heat_h,
                                        self.heat_w, num)
            target_batch_annos[:, :, start_index:end_index] = heat_map
        return target_batch_annos
