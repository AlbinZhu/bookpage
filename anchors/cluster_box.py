...
classfyFile = "./anchors/det_data.txt"
...
def avg_iou(boxes, centroids):
    ...
    for box in boxes:
        ...
        for centroid_index, centroid in enumerate(centroids):
            if 0:
                print("box.x %f, box.y: %f, box.w: %f, box.h:%f" %
                      (box.x, box.y, box.w, box.h))
                print(
                    "centroid.x %f, centroid.y: %f, centroid.w: %f, centroid.h:%f"
                    % (centroid.x, centroid.y, centroid.w, centroid.h))
...
def bboxIou(bbox_one, bbox_two):
    ...
    if (center_x_two -
            bbox_two_width / 2) < (center_x_one + bbox_one_width / 2) and (
                center_y_two - bbox_two_height / 2) < (center_y_one +
                                                       bbox_one_height / 2):
        jessord_area = (center_x_one + bbox_one_width / 2 -
                        (center_x_two - bbox_two_width / 2)) * (
                            center_y_one + bbox_one_height / 2 -
                            (center_y_two - bbox_two_height / 2))
...
def getClassflyIouBbox(annoBboxDatafile):
    ...
    with open(annoBboxDatafile, 'r') as file_:
        while True:
            ...
            if lineinfo[0] == '':
                break
            class_width = float(lineinfo[0])
            class_height = float(lineinfo[1])
...
def compute_centroids(label_path, n_anchors, loss_convergence, grid_size,
                      iterations_num, plus):
    ...
    # iterate k-means
    centroids, groups, old_loss, distance_all_old = do_kmeans(
        n_anchors, boxes, centroids)
    iterations = 1
    prev_assignments = np.ones(len(boxes)) * (-1)
    while (True):
        centroids, groups, loss, distance_all = do_kmeans(
            n_anchors, boxes, centroids)
        assignments = np.argmin(distance_all, axis=1)
        iterations = iterations + 1
        print("~~~~~~~~~~~~~~~the %d times iterations~~~~~~~~~~~~~~~~~~~~~~~" %
              (iterations + 1))
        print("old_loss = %f, loss = %f" % (old_loss, loss))
        if abs(old_loss -
               loss) < loss_convergence or iterations > iterations_num:
...
def main(args):
    if 1:
        ...
        plus = 1
        compute_centroids(classfyFile, n_anchors, loss_convergence, grid_size,
                          iterations_num, plus)
